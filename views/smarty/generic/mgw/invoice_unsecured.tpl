...
[{capture name="doNotShow"}]
    <script type="text/javascript">
        [{capture name="javaScript"}]
            function initUnzerInvoice_[{$paymentId|escape:'url'}]() {

                const unzerInstance = getUnzerInstance();
                if (!unzerInstance) {
                    setTimeout(initUnzerInvoice_[{$paymentId|escape:'url'}], 50);
                    return;
                }

                const invoice = unzerInstance.Invoice();

                if (!window.unzerPayments) {
                    window.unzerPayments = {};
                }

                window.unzerPayments['[{$paymentId|escape:'url'}]'] = invoice;
            }

            document.addEventListener("DOMContentLoaded", function() {
                initUnzerInvoice_[{$paymentId|escape:'url'}]();
            });
