...
[{capture name="doNotShow"}]
    <script type="text/javascript">
        [{capture name="javaScript"}]
            function initUnzerPrepayment_[{$paymentId|escape:'url'}]() {

                const unzerInstance = getUnzerInstance();
                if (!unzerInstance) {
                    setTimeout(initUnzerPrepayment_[{$paymentId|escape:'url'}], 50);
                    return;
                }

                const prepayment = unzerInstance.Prepayment();

                if (!window.unzerPayments) {
                    window.unzerPayments = {};
                }

                window.unzerPayments['[{$paymentId|escape:'url'}]'] = prepayment;
            }

            document.addEventListener("DOMContentLoaded", function() {
                initUnzerPrepayment_[{$paymentId|escape:'url'}]();
            });
